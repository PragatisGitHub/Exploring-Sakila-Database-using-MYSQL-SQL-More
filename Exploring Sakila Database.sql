...
first name, "Joe." What is one query would you use 
...
/* 2b. Find all actors whose last name contain the letters
 `GEN`:*/
...
/* 3b. You realize that some of these actors have 
tremendously long last names. Change the data type 
...
who have that last name, but only for names 
...
;
/* 4d. Perhaps we were too hasty in changing `GROUCHO` to 
`HARPO`. It turns out that `GROUCHO` was the correct name 
after all! In a single query, if the first name of the 
actor is currently `HARPO`, change it to `GROUCHO`. 
Otherwise, change the first name to `MUCHO GROUCHO`, 
as that is exactly what the actor will be with the grievous 
error. BE CAREFUL NOT TO CHANGE THE FIRST NAME OF EVERY ACTOR 
TO `MUCHO GROUCHO`, 
...
FROM actor WHERE last_name = "Williams"
;
/*The above excercise was discussed to be left*/
...
;
-- Above query gives the query that was used to create the schema
-- of the address table. We can use the same query to re-create
-- the schema.
...
for that film. Use tables `film_actor` and `film`. 
Use inner join.*/
...
/* 8a. In your new role as an executive, you would like to
 ...
 revenue. Use the solution from the problem above to 
